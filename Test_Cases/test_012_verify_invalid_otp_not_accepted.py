...
def test_invalid_otp_functionality(driver, config):
    url = config["app"]["url"]
    ...
    otp = "111111"
    logger.info(f"[STEP] Entering a random invalid OTP i.e.,OTP: {otp}...")
    ...
    logger.info("[STEP] Waiting for invalid OTP msg..")

    invalid_otp_msg = locate_element(driver, (By.XPATH, "//div[normalize-space()='Entered Invalid OTP']")
                                     )
    assert invalid_otp_msg is not None, "Invalid OTP msg not found"

    logger.info("[SUCCESS] 'Entered invalid otp' error populated as expected! Test Passed")
    time.sleep(delay)
