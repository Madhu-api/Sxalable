...
def test_signin_using_non_existing_email(driver, config):
    url = config["app"]["url"]
    ...
    xpath = "//div[@role='alert']//div[normalize-space()='User not found']"
    ...
    # Locating 'User Not found' error
    result = locate_element(driver, (By.XPATH, xpath))
    assert result is not None, "Unable to locate User Not Found error"
    logger.info(f"[INFO] located User Not Found error as expected...")
    time.sleep(delay)
