...
# Note: This TC will set up successful subscription, fresh sign up required after this for further TCs
def test_signin_mandatory_card_details(driver, config):
    url = config["app"]["url"]
    ...
    error_msg_xpath = "//div[@role='alert']//div[normalize-space()='Validation failed']"
    ...
    logger.info(f"[STEP] Entering Card Details into the form (for validation)...")
    try:
        for i, step in enumerate(card_fields):
            ...
            if i < len(card_fields) - 1:
                ...
                logger.info("[SUCCESS] Form valid, All card fields entered, user fields error cleared.")
            time.sleep(delay)
