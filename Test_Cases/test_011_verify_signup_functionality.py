...
def test_signup_functionality(driver, config):
    url = config["app"]["url"]
    ...
    expected_landing_url = "https://stage.sxalable.io/devices/sxr"
    ...
    # Verify URL
    url_contains = verify_url_contains(driver, expected_landing_url)
    assert url_contains, f"Failed to redirect to SxR Page. Current URL: {driver.current_url}"

    logger.info("[SUCCESS] Page navigated..")
    time.sleep(delay)

    # Locating Sign up Text
    text_found = verify_text_on_page(driver, 'Enter your details', timeout=15)
    assert text_found, f"'Enter your details' was NOT displayed in the page as expected"

    # store this email and workspace name for login for sign in test cases as sign up is
    # successful

    logger.info("[STEP] store this email and workspace name for login for further sign in test cases")
    ...
    logger.info(f"[INFO] Test data (email and workspace) saved at: {file_path} for further use")
