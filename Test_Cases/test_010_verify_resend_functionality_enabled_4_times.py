...
def test_resend_enabled_4_times(driver, config):
    url = config["app"]["url"]
    ...
    logger.info("[STEP] Waiting for 60 sec For resend button to be enabled..")

    time.sleep(60)
    logger.info("[STEP] Starting resend OTP count validation")
    for count in range(4, -1, -1):
        try:
            logger.info(f"iteration: {count}")
            xpath = f"//div[starts-with(normalize-space(),'Resend Code ({count} attempts left)')]"
            resend_btn = driver.find_element(By.XPATH, xpath)
            logger.info(f"[SUCCESS] Resend Code {count} attempts left button found as expected")
            time.sleep(delay)
            logger.info("[STEP] Clicking Resend..")
            resend_btn.click()
            logger.info("[STEP] Waiting for 60 sec..")
            time.sleep(60)

            if count == 0:
                resend_btn.click()
                logger.info(f"[DEBUG] Resend Click attempted at {count}th iteration")
                is_absent = verify_element_absent(driver, (By.XPATH, "//*[contains(text(),'OTP is shared')]"),
                                                   )
                assert is_absent, "OTP resend triggered unexpectedly even after 4 attempts"
                logger.info("[SUCCESS] Verified no more OTP shared at 0 attempts. ✅")

        except Exception as e:
            logger.error("[ERROR] Resend OTP count validation failed ❌")
            logger.error(str(e))
            assert False, f"Resend OTP count test failed: {str(e)}"
